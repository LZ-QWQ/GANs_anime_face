...
if __name__=='__main__':
    ...
    for i in range(startpage,numpages - 1):
        ...
        url = 'https://safebooru.org//index.php?page=dapi&s=post&q=index&pid=%d' % i
        while(1):#就手动打断吧
            try:
                html = requests.get(url,proxies,timeout=30)#,timeout=15)
                soup = BeautifulSoup(html.text, 'html.parser')
                #print(soup)
                temps=soup.find_all('post')
                break
            except KeyboardInterrupt:
                raise KeyboardInterrupt
            except:
