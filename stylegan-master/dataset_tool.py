...
def create_from_images(tfrecord_dir, image_dir, shuffle):
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            temp_filename=image_filenames[order[idx]]
            print(temp_filename)#输出文件名~
            try:#emmm报错气死人 我去处理截头的时候了emmm
                img = np.asarray(PIL.Image.open(temp_filename))
            except FileNotFoundError:
                print('上面这个读取失败，怀疑是文件名太长了QAQ')
                continue
            if channels == 1:
